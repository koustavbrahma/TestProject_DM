...
uniform sampler2D u_TextureUnit_Array[6];
...
void main() {
  ...
  vec3 amb, diff, spec;
  vec4 textColor;
  int index = int (v_TextureCoordinates.z);

  textColor = texture2D(u_TextureUnit_Array[index], v_TextureCoordinates.xy);

  phongModel(amb, diff, spec);
  float visibility = unpack();

  gl_FragColor = vec4(amb, 1.0) * textColor+ vec4(visibility * diff, 1.0) * textColor + vec4(visibility * spec, 1.0);
}